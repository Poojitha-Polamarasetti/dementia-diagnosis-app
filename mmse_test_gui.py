...
class MMSETestApp(tk.Tk):
    def __init__(self):
        ...
        self.container = tk.Frame(self, bg="#efb997")
    ...
    def show_question(self):
        ...
        tk.Label(self.container, text=f"Q{self.current_question+1}: {q['question']}", 
                 font=("Arial", 16), wraplength=600, bg="#efb997").pack(pady=60)
        ...
        if q["type"] == "entry":
            ...
            label = tk.Label(self.container, text="WORLD", font=("Arial", 14, "bold"), fg="blue", bg="#efb997")
            label.pack()
            self.user_input = tk.Entry(self.container, font=("Arial", 14))
            self.user_input.pack(pady=10)
            ...
            if q.get("image"):
                try:
                    ...
                    tk.Label(self.container, image=img, bg="#efb997").pack()
                    self.container.image = img
                except:
                    tk.Label(self.container, text="[Image not available]", font=("Arial", 12), bg="#efb997").pack()
            for opt in q["options"]:
                tk.Radiobutton(self.container, text=opt.title(), variable=self.var, value=opt.lower(), 
                               bg="#efb997", font=("Arial", 14)).pack(anchor="w")
            ...
            for opt in q["options"]:
                var = tk.IntVar()
                self.vars[opt] = var
                tk.Checkbutton(self.container, text=opt, variable=var, font=("Arial", 14), bg="#efb997").pack(anchor="w")
        ...
        tk.Button(self.container, text="Next", command=self.evaluate_answer, font=("Arial", 14), 
                  bg="#2196F3", fg="white").pack(pady=30)
