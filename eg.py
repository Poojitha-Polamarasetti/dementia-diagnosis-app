...
class MainPage(BasePage):
    def __init__(self, parent, controller):
        ...
        # Left logo
        try:
            self.left_logo = Image.open("gvp logo.png")
            self.left_logo_img = ImageTk.PhotoImage(self.left_logo)
            self.left_logo_label = ctk.CTkLabel(self.top_frame, image=self.left_logo_img, text="")
            self.left_logo_label.pack(side="left", padx=10)
        except:
            self.left_logo_label = ctk.CTkLabel(self.top_frame, text="[LOGO]", width=100, height=100)
            self.left_logo_label.pack(side="left", padx=10)
        
        # Center labels
        self.title_frame = ctk.CTkFrame(self.top_frame, fg_color="transparent")
        self.title_frame.pack(side="left", expand=True)
        
        self.univ_label = ctk.CTkLabel(self.title_frame, 
                     text="GAYATRI VIDYA PARISHAD",
                     font=ctk.CTkFont(family="Courier", size=22, weight="bold"))
        self.univ_label.pack()
        
        self.college_label = ctk.CTkLabel(self.title_frame, 
                     text="COLLEGE FOR DEGREE AND PG COURSES (A)",
                     font=ctk.CTkFont(family="Courier", size=18))
        self.college_label.pack()
        
        self.dept_label = ctk.CTkLabel(self.title_frame, 
                     text="Department of Computer Applications",
                     font=ctk.CTkFont(family="Times New Roman", size=14, weight="bold"))
        self.dept_label.pack()
        
        # Right logo
        try:
            self.right_logo = Image.open("gvp logo.png")
            self.right_logo_img = ImageTk.PhotoImage(self.right_logo)
            self.right_logo_label = ctk.CTkLabel(self.top_frame, image=self.right_logo_img, text="")
            self.right_logo_label.pack(side="right", padx=10)
        except:
            self.right_logo_label = ctk.CTkLabel(self.top_frame, text="[LOGO]", width=100, height=100)
            self.right_logo_label.pack(side="right", padx=10)
        
        # Project Title
        self.project_title = ctk.CTkLabel(self.container, 
                     text="Initial Diagnosis of Dementia using Machine Learning Approach",
        ...
        self.author_frame = ctk.CTkFrame(self.content_frame, fg_color="transparent")
        self.author_frame.pack(side="left", padx=40)
        
        self.author_label = ctk.CTkLabel(self.author_frame, 
                     text="By: P. Poojitha\nPG232402030\nMaster of Computer Applications\nSem-IV",
                     justify="left")
        self.author_label.pack()
        
        self.guide_frame = ctk.CTkFrame(self.content_frame, fg_color="transparent")
        self.guide_frame.pack(side="right", padx=40)
        
        self.guide_label = ctk.CTkLabel(self.guide_frame, 
        ...
        self.proceed_btn = ctk.CTkButton(self.bottom_frame, text="Proceed", width=150,
                      command=lambda: controller.show_frame(SecondPage))
        self.proceed_btn.pack(side="left", padx=20)
        
        self.exit_btn = ctk.CTkButton(self.bottom_frame, text="Exit", width=150, fg_color="#d9534f",
    ...
    def on_resize(self, event):
        ...
        if width < 1000 or height < 700:
            # Smaller window - reduce font sizes
            self.univ_label.configure(font=ctk.CTkFont(family="Courier", size=18, weight="bold"))
            self.college_label.configure(font=ctk.CTkFont(family="Courier", size=14))
            self.dept_label.configure(font=ctk.CTkFont(family="Times New Roman", size=12, weight="bold"))
            self.project_title.configure(font=ctk.CTkFont(family="Times New Roman", size=16, weight="bold"))
            
            # Adjust logo sizes
            try:
                new_size = (80, 80)
                left_logo = self.left_logo.resize(new_size, Image.LANCZOS)
                self.left_logo_img = ImageTk.PhotoImage(left_logo)
                self.left_logo_label.configure(image=self.left_logo_img)
                
                right_logo = self.right_logo.resize(new_size, Image.LANCZOS)
                self.right_logo_img = ImageTk.PhotoImage(right_logo)
                self.right_logo_label.configure(image=self.right_logo_img)
            except:
                pass
            
            # Adjust button sizes
            self.proceed_btn.configure(width=120)
            self.exit_btn.configure(width=120)
        else:
            # Larger window - normal font sizes
            self.univ_label.configure(font=ctk.CTkFont(family="Courier", size=22, weight="bold"))
            self.college_label.configure(font=ctk.CTkFont(family="Courier", size=18))
            self.dept_label.configure(font=ctk.CTkFont(family="Times New Roman", size=14, weight="bold"))
            self.project_title.configure(font=ctk.CTkFont(family="Times New Roman", size=20, weight="bold"))
            
            # Reset logo sizes
            try:
                original_size = (100, 100)
                left_logo = self.left_logo.resize(original_size, Image.LANCZOS)
                self.left_logo_img = ImageTk.PhotoImage(left_logo)
                self.left_logo_label.configure(image=self.left_logo_img)
                
                right_logo = self.right_logo.resize(original_size, Image.LANCZOS)
                self.right_logo_img = ImageTk.PhotoImage(right_logo)
                self.right_logo_label.configure(image=self.right_logo_img)
            except:
                pass
            ...
            # Reset button sizes
            self.proceed_btn.configure(width=150)
            self.exit_btn.configure(width=150)
