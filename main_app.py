...
class MainPage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        # Left Logo
        try:
            left_logo = Image.open("images/gvp logo.png").resize((150, 150))
        ...
        # Right Logo
        try:
            right_logo = Image.open("images/gvp logo.png").resize((150, 150))
        ...
        proceed_btn = ctk.CTkButton(self, text="Proceed", width=180,height=50,
                                    command=lambda: controller.show_frame(NamePage))
        proceed_btn.pack(side="left", padx=50)

        exit_btn = ctk.CTkButton(self, text="Exit", width=180,height=50, fg_color="#d9534f",
...
class NamePage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        ctk.CTkLabel(title_frame, text="COLLEGE FOR DEGREE AND PG COURSES (A)", 
                     font=("Times New Roman", 36, "bold"), text_color="black").pack(pady=(0, 5))
        ctk.CTkLabel(title_frame, text="Department of Computer Science", 
                     font=("Times New Roman", 30), text_color="black").pack(pady=(0, 10))
        ...
        ctk.CTkLabel(left_box, text="By:", font=("Arial", 20, "bold"), text_color="black").pack(pady=2)
        ctk.CTkLabel(left_box, text="K. Harshini", font=("Arial", 20), text_color="black").pack(pady=2)
        ctk.CTkLabel(left_box, text="PG232406029 – M.Sc.(c.s)", font=("Arial", 20), text_color="black").pack(pady=2)
        ctk.CTkLabel(left_box, text="Sem- IV", font=("Arial", 20), text_color="black").pack(pady=2)
        ...
        proceed_btn.pack(side="bottom", padx=50)
    ...
    def check_login(self):
        ...
        try:
            ...
            if conn:
                ...
                if result:
                    user_id = result[0]
                    self.controller.current_user_id = user_id  # Store user ID
...
class RegisterPage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        bullets_frame = ctk.CTkFrame(self, fg_color="transparent")
        ...
        for b in bullets:
            ctk.CTkLabel(bullets_frame, text=b, font=ctk.CTkFont(size=14), text_color="gray").pack(anchor="center")
...
class MedicalTestsPage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        #self.submit_button = ctk.CTkButton(self.button_panel, text="Submit", width=120, command=self.submit_data)
        ...
        self.back_button = ctk.CTkButton(self.button_panel, text="Back", width=120, fg_color="#d9534f",
                                        command=lambda: self.controller.show_frame(TitlePage))
    ...
    def validate_age_input(self, *args):
        ...
        self.age_validation_job = self.after(500, self.perform_age_check, value)
...
class ResultPage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        user_fields = ["Name", "Age", "Gender", "MMSE", "BMI", "Smoking", "Alcohol", "Activity", "Sleep Hours"]
        self.user_labels = {}
        ...
        self.model_accuracy_label = ctk.CTkLabel(self.summary_card, text="Model Accuracy: 94.00%", font=font_label)
        ...
        self.back_btn = ctk.CTkButton(self.button_frame, text="Back",fg_color="#422ec3", hover_color="#422ec3", command=lambda: controller.show_frame(LoginPage))
...
class HistoryPage(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        # Date Filter Entry
        self.date_filter_entry = ctk.CTkEntry(self.filter_frame, width=200, placeholder_text="YYYY-MM-DD")
        self.date_filter_entry.pack(side="left", padx=(0, 10))

        # Age Filter Entry
        self.age_filter_entry = ctk.CTkEntry(self.filter_frame, width=100, placeholder_text="Age")
        self.age_filter_entry.pack(side="left", padx=(10, 10))

        # Diagnosis Filter Entry
        self.diagnosis_filter_entry = ctk.CTkEntry(self.filter_frame, width=120, placeholder_text="Diagnosis")
        self.diagnosis_filter_entry.pack(side="left", padx=(0, 10))


        # Search Button
        self.search_btn = ctk.CTkButton(self.filter_frame, text="Filter", command=self.apply_date_filter, width=100)
        self.search_btn.pack(side="left")

        # Show All Button
        self.reset_btn = ctk.CTkButton(self.filter_frame, text="Reset", command=self.load_login_history, width=100)
...
# === Treeview (Table) ===
    ...
    def apply_date_filter(self):
        target_date = self.date_filter_entry.get().strip()
        age_filter = self.age_filter_entry.get().strip() if hasattr(self, 'age_filter_entry') else ""
        diagnosis_filter = self.diagnosis_filter_entry.get().strip() if hasattr(self, 'diagnosis_filter_entry') else ""
        ...
        try:
            ...
            # Base query
            query = """
                SELECT lh.login_timestamp, p.age, p.bmi, mh.diagnosis
                FROM login_history lh
                LEFT JOIN patients p ON lh.user_id = p.user_id
                LEFT JOIN (
                    SELECT *
            ...
            if target_date:
                try:
                    ...
                    conditions.append("DATE(lh.login_timestamp) = %s")
            ...
            if diagnosis_filter:
                conditions.append("mh.diagnosis LIKE %s")
                params.append(f"%{diagnosis_filter}%")
            ...
            self.tree.delete(*self.tree.get_children())
            login_count = 0
            for ts, age, bmi, diagnosis in rows:
                date_str = ts.strftime("%Y-%m-%d")
                time_str = ts.strftime("%H:%M:%S")
            ...
            if login_count == 0:
                self.stats_label.configure(text="No results found for the given filters.")
            else:
                self.stats_label.configure(text=f"Filtered Results: {login_count}")
